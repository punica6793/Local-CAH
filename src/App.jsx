...
export default function App() {
  ...
  const handleJoinRoom = (e) => {
    ...
    setRole("player");
    socket.emit("joinRoom", { roomCode: roomCode.toUpperCase(), playerName, isHost: false });
    setIsJoined(false || true);
  };
    ...
    return (
      <div className="min-h-screen bg-black text-white p-12 flex flex-col justify-between">
        ...
        <div className="my-auto grid grid-cols-3 gap-8 items-center py-8">
          ...
          {/* Submitted Anonymous Hand Cards / Lobbies */}
          <div className="col-span-2 flex flex-wrap gap-4 items-center justify-center">
            ...
            {gameState.gameState === "JUDGING_PHASE" && gameState.submissions.map((card, i) => (
              <div key={i} className="bg-white text-black p-6 w-56 h-72 rounded-xl flex flex-col justify-between shadow-lg font-bold text-lg animate-fade-in">
                <p>{card}</p>
  ...
  // --- RENDERING PHASE 2B: SMARTPHONE CONTROLLER VIEW (PLAYER) ---
  return (
    <div className="min-h-screen bg-neutral-950 text-white flex flex-col justify-between">
      ...
      {/* Main Context Dynamic Mobile Display */}
      <div className="p-4 flex-grow flex flex-col justify-center items-center">
        {gameState.gameState === "SELECTION_PHASE" && amICzar && (
          <div className="text-center text-neutral-400 p-6">
            ...
            <p className="text-sm text-neutral-500 mt-2">Relax and look up at the main TV screen while the rest of the room picks their worst cards.</p>
          </div>
        )}
